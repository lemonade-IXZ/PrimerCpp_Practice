...
进度：1.5节练习/全19章；2024.3.27；

待办：

- [ ] 1.10解释++的前置与后置用法区别；

- [ ] 1.11程序里忘记写return 0会怎样；
...
练习1.20： 在网站 http://www.informit.com/title/0321714113 上，第1章的代码目录中包含了头文件Sales_item.h。
...
我的想法是用示例程序，毕竟它都可以使不同ISBN的对象相加；

参考解答我认为超纲了，我想过比较ISBN，但书上没讲；

之后碰到类似情况先试着搜索看看，不行再看答案；
...
0-201-78345-X 4 15.00

预计输出：

0-201-78345-X 10 200 20

解答记录：

考虑第一条输入作为初始化不进入循环，但这样使输入变麻烦了；

意外发现有默认初始化，值为0，0，0；

用循环修改后不能实现功能了，打算参考书本解答修改程序；

注意到使用了“return -1”来退出程序；

解答还考虑了没有数据的情况，我认为是考虑到包含了错误输入在内的其他情况；确实需要一个兜底的选择语句；
...
预期：1，2，3，1，1（没有考虑不连续情况，即未寄存所有书号）
...
我在上个练习已经编写了记录，还升级了记录的难度给上个程序带来更大的挑战；

书本解答：在网站 http://www.informit.com/title/0321714113 上，第1章的代码目录中包含了一些数据文件，
可以将这些文件重定向到此程序进行测试，也可自己创建销售记录文件进行测试。

ps：没有购买本书似乎不能查看数据文件；

## 1.6 书店程序

需要假定每个ISBN书号的所有销售记录在文件中是聚在一起保存的；（看来保存和检索所有书号在目前比较困难）
...
书本解答：在网站 http://www.informit.com/title/0321714113 上下载Sales_ item.h头文件，编译书店程序即可。
